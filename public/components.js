...
function loadMaterials() {
  ...
  fetch('/api/wide-format-materials').then(function(r){ return r.ok ? r.json() : []; })
    .then(function(data){
...
function buildMaterialOptions(selectedId) {
  ...
  if (!cachedMaterials.length) {
    return '<option value="">No materials in wide_format_materials</option>';
  }
  var html = '<option value="">— Select material —</option>';
  cachedMaterials.forEach(function(m){
    var sel = String(m.material_code) === String(selectedId) ? ' selected' : '';
    var w = m.width_in ? Number(m.width_in) : null;
    var lenFt = m.length_ft ? Number(m.length_ft) : null;
    var sz = w ? ' (' + w + '"' + (lenFt ? ' × ' + lenFt + "'" : '') + ')' : '';
    var price = ' — $' + Number(m.unit_cost||0).toFixed(4) + '/sqft';
    var label = (m.material_code ? m.material_code + ' — ' : '') + (m.description || '');
    html += '<option value="' + escHtml(m.material_code) + '"' + sel + '>' + escHtml(label) + sz + price + '</option>';
  });
  return html;
}
...
window.applyMaterial = function(id) {
  ...
  var m = (cachedMaterials||[]).find(function(x){ return String(x.material_code) === String(id); });
  if (!m) return;
  l.material_id = m.material_code;
  l.material_name = m.description || m.material_code;
  if (m.width_in)  l.sw = parseFloat(m.width_in);
  if (m.length_ft) l.sh = parseFloat(m.length_ft) * 12;
  if (m.unit_cost != null) l.csf = parseFloat(m.unit_cost) || 0;
  renderCompEditor();
};
