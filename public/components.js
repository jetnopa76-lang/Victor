...
// Press presets — Tauro H2500 modes. Speeds are sqft/hour. Rate is $/hour of press time.
// Per-sqft rate is computed as (rate_per_h / speed_sqft_h). Edit per row after adding.
var PRESS_PRESETS = [
  { id: 'tauro_express',  label: 'Tauro H2500 — Express',          speed_sqft_h: 4198, rate_per_h: 90, note: '390 m²/h · production speed' },
  { id: 'tauro_standard', label: 'Tauro H2500 — Standard',         speed_sqft_h: 2153, rate_per_h: 90, note: '200 m²/h · standard quality' },
  { id: 'tauro_quality',  label: 'Tauro H2500 — Quality',          speed_sqft_h: 1076, rate_per_h: 90, note: '100 m²/h · high quality' },
  { id: 'tauro_premium',  label: 'Tauro H2500 — Premium / Backlit', speed_sqft_h:  538, rate_per_h: 90, note: '50 m²/h · premium / backlit' }
];
...
var TAB_LIBRARY_MAP = [
  ...
  { match: /^press$|^\s*press\s*$|press(?!.*post)/i, presets: PRESS_PRESETS, label: 'Press mode' }
];
...
function buildRowMaterialOptions(lib, item) {
  if (lib.presets) {
    ...
    lib.presets.forEach(function(p){
      var sel = item && p.id === item.preset_id ? ' selected' : '';
      html += '<option value="preset:' + p.id + '"' + sel + '>' + escHtml(p.label) + ' — $' + p.rate_per_h + '/hr</option>';
    });
    return html;
  }
...
window.applyRowMaterial = function(tabIdx, itemIdx, value) {
  ...
  if (typeof value === 'string' && value.indexOf('preset:') === 0) {
    ...
    var p = PRESS_PRESETS.find(function(x){ return x.id === pid; });
    if (!p) return;
    ...
    item.rate = p.speed_sqft_h > 0 ? +(p.rate_per_h / p.speed_sqft_h).toFixed(4) : 0;
