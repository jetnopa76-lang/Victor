...
// Presses — add new ones by appending to this array. Each press has multiple modes.
// Speeds are sqft/hour. rate_per_h is the press's hourly operating cost.
// Per-row rate becomes rate_per_h / speed_sqft_h for the chosen mode.
var PRESSES = [
  {
    id: 'tauro_h2500',
    name: 'Tauro H2500',
    rate_per_h: 90,
    modes: [
      { id: 'express',  label: 'Express',          speed_sqft_h: 4198, note: '390 m²/h' },
      { id: 'standard', label: 'Standard',         speed_sqft_h: 2153, note: '200 m²/h' },
      { id: 'quality',  label: 'Quality',          speed_sqft_h: 1076, note: '100 m²/h' },
      { id: 'premium',  label: 'Premium / Backlit', speed_sqft_h:  538, note: '50 m²/h' }
    ]
  },
  {
    id: 'nyala_4',
    name: 'swissQprint Nyala 4',
    rate_per_h: 90,
    modes: [
      { id: 'pop',         label: 'POP / Speed',       speed_sqft_h: 2217, note: '206 m²/h' },
      { id: 'production',  label: 'Production',        speed_sqft_h: 1292, note: '120 m²/h' },
      { id: 'quality',     label: 'Quality',           speed_sqft_h:  646, note: '60 m²/h' },
      { id: 'backlit',     label: 'Backlit / Premium', speed_sqft_h:  323, note: '30 m²/h' }
    ]
  }
  // Future presses: add objects here with { id, name, rate_per_h, modes:[...] }
];

// Flatten for quick lookup by "pressId:modeId"
function getPressPreset(presetId) {
  if (!presetId) return null;
  var parts = presetId.split(':');
  if (parts.length !== 2) return null;
  var press = PRESSES.find(function(p){ return p.id === parts[0]; });
  if (!press) return null;
  var mode = press.modes.find(function(m){ return m.id === parts[1]; });
  if (!mode) return null;
  return { press: press, mode: mode };
}
...
var TAB_LIBRARY_MAP = [
  ...
  { match: /^press$|^\s*press\s*$|press(?!.*post)/i, presses: PRESSES, label: 'Press mode' }
];
...
function buildRowMaterialOptions(lib, item) {
  if (lib.presses) {
    var html = '<option value="">— Select press / mode —</option>';
    lib.presses.forEach(function(press){
      html += '<optgroup label="' + escHtml(press.name) + ' ($' + press.rate_per_h + '/hr)">';
      press.modes.forEach(function(mode){
        var pid = press.id + ':' + mode.id;
        var sel = item && pid === item.preset_id ? ' selected' : '';
        var rate = press.rate_per_h / mode.speed_sqft_h;
        html += '<option value="preset:' + pid + '"' + sel + '>' + escHtml(mode.label) + (mode.note ? ' (' + escHtml(mode.note) + ')' : '') + ' · $' + rate.toFixed(4) + '/sqft</option>';
      });
      html += '</optgroup>';
    });
    return html;
      ...
      if (ccItems.length) {
        ...
        Object.keys(groups).sort().forEach(function(g){
          ...
          groups[g].forEach(function(x){
            ...
            var label = (x.code ? x.code + ' — ' : '') + (x.name || '');
...
window.applyRowMaterial = function(tabIdx, itemIdx, value) {
  ...
  // Press preset path: "preset:pressId:modeId"
  if (typeof value === 'string' && value.indexOf('preset:') === 0) {
    var pid = value.substring(7);
    var pp = getPressPreset(pid);
    if (!pp) return;
    var lc = calcLayoutCost(components[currentCompIdx].layout);
    item.preset_id = pid;
    item.material_id = null;
    item.cost_center_item_id = null;
    item.name = pp.press.name + ' — ' + pp.mode.label;
    item.method = 'per_sqft';
    item.rate = pp.mode.speed_sqft_h > 0 ? +(pp.press.rate_per_h / pp.mode.speed_sqft_h).toFixed(4) : 0;
    item.qty = lc.sqft > 0 ? Math.ceil(lc.sqft) : 1;
    renderCompEditor();
    return;
  }
  // Cost-center item path: "cc:<id>"
  if (typeof value === 'string' && value.indexOf('cc:') === 0) {
    ...
    if (!cc) return;
    var qty = parseFloat(components[currentCompIdx].layout.qty) || 1;
    ...
    // Decide method/rate/qty based on which fields are populated
    if (speedPerH > 0 && rate > 0) {
      // Postpress run: cost = (qty/speed) * rate ; per-unit equivalent = rate/speed
      ...
      item.rate = rate;
      item.qty = +(minsPerUnit * qty / 60).toFixed(4);
    } else if (minCharge > 0) {
      item.method = 'flat';
      item.rate = minCharge;
      item.qty = 1;
    } else {
      // No data — leave as flat $0 for user to fill
      item.method = 'flat';
      item.rate = 0;
      item.qty = 1;
    }
    // If postpress with setup_min > 0, insert a separate setup row before this one
...
function renderProcessTab(c, idx) {
  ...
  var hasLib = lib && (
    (lib.presses && lib.presses.length) ||
