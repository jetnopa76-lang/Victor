...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .cw-tab{padding:9px 9px;font-size:12px;cursor:pointer;border-bottom:2px solid transparent;color:#888;background:none;border-top:none;border-left:none;border-right:none;white-space:nowrap;flex:0 0 auto}
...
window.applyRowMaterial = function(tabIdx, itemIdx, value) {
  ...
  if (typeof value === 'string' && value.indexOf('cc:') === 0) {
    ...
    if (cc.cc_kind === 'lamination') {
      ...
      item.min_charge = parseFloat(cc.min_charge) || 0;
      delete item.material; delete item.method; delete item.rate; delete item.qty;
      renderCompEditor();
      return;
    }
    item.cost_center_item_id = cc.id;
