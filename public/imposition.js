...
async function populateDropdowns() {
  ...
  if (ipMedia) {
    try {
      const r = await fetch('/api/wide-format-materials');
      _wfMaterials = await r.json();
      ipMedia.innerHTML = '<option value="">— Select material —</option>' +
        _wfMaterials.map(function(m) {
          return '<option value="' + m.material_code + '">' +
            m.material_code + ' — ' + m.description +
            (m.width_in ? ' (' + m.width_in + '")' : '') +
            '</option>';
        }).join('');
      ipMedia.onchange = function() {
        var mat = _wfMaterials.find(function(x) { return x.material_code == ipMedia.value; });
        if (mat) {
          document.getElementById('ip_csf').value = parseFloat(mat.unit_cost).toFixed(4);
          if (mat.width_in) document.getElementById('ip_sw').value = mat.width_in;
          runImp();
        }
      };
    } catch(e) { console.error('Wide format materials load error:', e); }
  }
}
