...
function calc(){
  ...
  var compBD = (typeof getComponentsBreakdown === 'function') ? getComponentsBreakdown() : [];
  compBD.forEach(function(c){ cost += c.val; });
  ...
  // Commission is added to the customer total (passed through to customer, not absorbed by shop)
  var netProfit=profit;
  var total=sell+tax+comm;
...
window.exportCustomerCopy = function() {
  if (!lastCalc.total) { if (typeof toast === 'function') toast('Calculate the estimate first'); return; }
  ...
  var customer = (typeof customers !== 'undefined') ? customers.find(function(c){ return String(c.id) === String(custSel.value); }) : null;
  var jobType = document.getElementById('jobType').value;
  var qty = jobType === 'digital'
    ? (parseInt(document.getElementById('qty').value) || 1)
    : (parseInt(document.getElementById('wfQty').value) || 1);
  var compBD = (typeof getComponentsBreakdown === 'function') ? getComponentsBreakdown() : [];
  // Customer block
  var custLines = [];
  if (customer) {
    if (customer.company) custLines.push('<strong>'+esc(customer.company)+'</strong>');
    custLines.push(esc((customer.first_name||'')+' '+(customer.last_name||'')).trim());
    if (customer.address_line1) custLines.push(esc(customer.address_line1));
    if (customer.address_line2) custLines.push(esc(customer.address_line2));
    var cityStateZip = [customer.city, customer.state, customer.zip].filter(Boolean).join(', ');
    if (cityStateZip) custLines.push(esc(cityStateZip));
    if (customer.email) custLines.push(esc(customer.email));
    if (customer.phone) custLines.push(esc(customer.phone));
  } else {
    custLines.push('<em style="color:#aaa">Walk-in customer</em>');
  }
  var dateStr = new Date().toLocaleDateString();
  var bar = document.getElementById('estActionBar');
  var numEl = bar ? bar.querySelector('span[style*="monospace"]') : null;
  var estNumber = numEl ? numEl.textContent : '—';
  // Line items: scale component costs to sell ratio
  var sellRatio = lastCalc.cost > 0 ? (lastCalc.sell / lastCalc.cost) : 1;
  var items = [];
  if (compBD.length) {
    var compTotal = 0;
    compBD.forEach(function(c){
      compTotal += c.val;
      items.push({ desc: c.label, detail: c.detail, amt: c.val * sellRatio });
    });
    var otherCost = lastCalc.cost - compTotal;
    if (otherCost > 0.50) {
      items.push({ desc: 'Setup, materials and labor', detail: '', amt: otherCost * sellRatio });
    }
  } else {
    items.push({
      desc: jobName,
      detail: (jobType === 'digital' ? qty + ' pcs · digital print' : qty + ' pcs · wide format'),
      amt: lastCalc.sell
    });
  }
  var itemRows = items.map(function(it){
    return '<tr><td><strong>'+esc(it.desc)+'</strong>'+(it.detail?'<div class="detail">'+esc(it.detail)+'</div>':'')+'</td><td class="right">$'+it.amt.toFixed(2)+'</td></tr>';
  }).join('');
  var taxRow = lastCalc.taxPct > 0 ? '<tr><td>Tax ('+lastCalc.taxPct+'%)</td><td class="right">$'+lastCalc.tax.toFixed(2)+'</td></tr>' : '';
  var notesBlock = jobNotes ? '<div class="notes"><div class="notes-label">Notes</div>'+esc(jobNotes).replace(/\n/g, '<br>')+'</div>' : '';
  var html = '<!DOCTYPE html><html><head><title>Estimate '+esc(estNumber)+' — '+esc(jobName)+'</title>'+
    '<style>'+
    ...
    'h1{font-size:28px;font-weight:300;letter-spacing:2px;margin:0}'+
    '.header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:30px;padding-bottom:16px;border-bottom:2px solid #1a1a18}'+
    '.header-right{text-align:right;font-size:12px;color:#666}'+
    '.row{display:flex;gap:30px;margin-bottom:30px}'+
    '.col{flex:1}'+
    '.col-label{font-size:10px;text-transform:uppercase;letter-spacing:1px;color:#888;margin-bottom:6px}'+
    '.col-content{font-size:13px;line-height:1.6}'+
    'table{width:100%;border-collapse:collapse;margin:20px 0}'+
    'th{text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:1px;color:#888;font-weight:500;padding:10px 8px;border-bottom:1px solid #ddd}'+
    'th.right,td.right{text-align:right}'+
    'td{padding:10px 8px;border-bottom:1px solid #f0efec;font-size:13px;vertical-align:top}'+
    'td .detail{font-size:11px;color:#888;margin-top:2px}'+
    '.totals{margin-top:16px}'+
    '.totals tr td{border:none;padding:4px 8px;font-size:13px}'+
    '.totals tr.subtotal td{border-top:1px solid #ddd;padding-top:10px}'+
    '.totals tr.grand td{font-size:18px;font-weight:500;border-top:2px solid #1a1a18;padding-top:12px}'+
    '.notes{margin-top:30px;padding:16px;background:#f9f8f6;border-radius:8px;font-size:13px;line-height:1.6}'+
    '.notes-label{font-size:10px;text-transform:uppercase;letter-spacing:1px;color:#888;margin-bottom:6px}'+
    '.footer{margin-top:50px;padding-top:20px;border-top:1px solid #ddd;font-size:11px;color:#666;line-height:1.6}'+
    ...
    '@media print{body{padding:0.4in}.no-print{display:none}}'+
    '</style></head><body>'+
    ...
    '<div class="header"><div><h1>ESTIMATE</h1><div style="font-size:12px;color:#888;margin-top:4px">'+esc(estNumber)+'</div></div>'+
      '<div class="header-right"><div><strong style="font-size:14px;color:#1a1a18">'+esc(dateStr)+'</strong></div><div style="margin-top:4px">Valid 30 days</div></div></div>'+
    '<div class="row">'+
      '<div class="col"><div class="col-label">Bill to</div><div class="col-content">'+custLines.join('<br>')+'</div></div>'+
      '<div class="col"><div class="col-label">Job</div><div class="col-content"><strong>'+esc(jobName)+'</strong></div></div>'+
    '</div>'+
    '<table><thead><tr><th>Description</th><th class="right" style="width:120px">Amount</th></tr></thead><tbody>'+itemRows+'</tbody></table>'+
    '<table class="totals">'+
      '<tr class="subtotal"><td>Subtotal</td><td class="right">$'+lastCalc.sell.toFixed(2)+'</td></tr>'+
      taxRow+
      '<tr class="grand"><td>TOTAL</td><td class="right">$'+lastCalc.total.toFixed(2)+'</td></tr>'+
    '</table>'+
    notesBlock+
    '<div class="footer">This is an estimate. Actual costs may vary based on final specifications. Quote is valid for 30 days from the date above. Please contact us with any questions.</div>'+
    '</body></html>';
  var w = window.open('', '_blank', 'width=900,height=1000');
  if (!w) { if (typeof toast === 'function') toast('Allow popups to export'); return; }
