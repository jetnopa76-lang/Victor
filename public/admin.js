  ...
  if(!sel){
    ...
    if(items.length){
      if(deptModel==='prepress'){
        ...
        // Lamination model: Labor $/sq ft + Material $/sq ft (+ Min $)
        procHTML+='<div class="_lc" style="margin-bottom:8px"><div style="display:grid;grid-template-columns:60px 1fr 90px 90px 60px 24px;gap:5px;padding:6px 10px;border-bottom:1px solid #e8e6e2">'+th+'Code</span>'+th+'Name</span>'+th+'Labor $/sqft</span>'+th+'Material $/sqft</span>'+th+'Min $</span><span></span></div>'+items.map(function(x){return '<div class="_mr" style="grid-template-columns:60px 1fr 90px 90px 60px 24px"><input value="'+_e(x.code||'')+'" onchange="_uCCI('+x.id+',\'code\',this.value)"><input value="'+_e(x.name)+'" onchange="_uCCI('+x.id+',\'name\',this.value)"><input type="number" value="'+(parseFloat(x.sqft_rate)||0).toFixed(4)+'" step="0.0001" onchange="_uCCI('+x.id+',\'sqft_rate\',this.value)"><input type="number" value="'+(parseFloat(x.dm_rate)||0).toFixed(4)+'" step="0.0001" onchange="_uCCI('+x.id+',\'dm_rate\',this.value)"><input type="number" value="'+(parseFloat(x.min_charge)||0).toFixed(2)+'" step="0.5" onchange="_uCCI('+x.id+',\'min_charge\',this.value)"><button style="background:none;border:none;cursor:pointer;color:#ccc;font-size:15px;padding:0" onclick="_dCCI('+x.id+')">×</button></div>';}).join('')+'</div>';
      }else{
