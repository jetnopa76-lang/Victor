...
// COST CENTERS
var _cck='prepress'; // current kind
var _cccid=null;     // current center id
async function _rCC(){
  ...
  var kindToggle='<div style="display:flex;gap:6px;margin-bottom:10px"><button class="'+(_cck==='prepress'?'_btn':'_btnG')+'" style="flex:1" onclick="_sCCK(\'prepress\')">Prepress</button><button class="'+(_cck==='press'?'_btn':'_btnG')+'" style="flex:1" onclick="_sCCK(\'press\')">Press</button><button class="'+(_cck==='postpress'?'_btn':'_btnG')+'" style="flex:1" onclick="_sCCK(\'postpress\')">Postpress</button></div>';
  var cH=kindToggle+centers.map(function(c){var a=c.id==_cccid;return '<div class="_mc'+(a?' _mca':'')+'" onclick="_sCC('+c.id+')"><span><span style="font-family:monospace;color:'+(a?'#ddd':'#aaa')+';font-size:11px;margin-right:6px">'+_e(c.code)+'</span>'+_e(c.name)+'</span><button style="background:none;border:none;cursor:pointer;font-size:14px;color:'+(a?'#aaa':'#ddd')+';padding:0" onclick="event.stopPropagation();_dCC('+c.id+')">×</button></div>';}).join('')+'<button onclick="_aCC()" style="width:100%;margin-top:6px;padding:8px;border:2px dashed #ddd;border-radius:8px;background:none;font-size:12px;color:#378ADD;cursor:pointer">+ Add cost center</button>';
  var iH='<div style="color:#aaa;font-size:13px;padding:10px 0">Select a cost center</div>';
  if(sel){
    var hdr='<div style="margin-bottom:12px"><div style="font-size:15px;font-weight:600"><span style="font-family:monospace;color:#888;font-size:13px;margin-right:8px">'+_e(sel.code)+'</span>'+_e(sel.name)+'</div><div style="font-size:12px;color:#888;margin-top:2px">'+sel.kind+' · '+items.length+' item(s)</div></div>';
    var rowsHTML='';
    if(items.length){
      if(_cck==='prepress'){
        // prepress columns: code/name/mins/AI/DM/unit/min
        rowsHTML='<div class="_lc" style="margin-bottom:8px"><div style="display:grid;grid-template-columns:60px 1fr 60px 60px 60px 60px 60px 28px;gap:5px;padding:6px 10px;border-bottom:1px solid #e8e6e2"><span style="font-size:10px;color:#aaa;text-transform:uppercase">Code</span><span style="font-size:10px;color:#aaa;text-transform:uppercase">Name</span><span style="font-size:10px;color:#aaa;text-transform:uppercase">Mins/Unit</span><span style="font-size:10px;color:#aaa;text-transform:uppercase">AI $/hr</span><span style="font-size:10px;color:#aaa;text-transform:uppercase">DM $/hr</span><span style="font-size:10px;color:#aaa;text-transform:uppercase">Unit $</span><span style="font-size:10px;color:#aaa;text-transform:uppercase">Min $</span><span></span></div>'+items.map(function(x){return '<div class="_mr" style="grid-template-columns:60px 1fr 60px 60px 60px 60px 60px 28px"><input value="'+_e(x.code||'')+'" onchange="_uCCI('+x.id+',\'code\',this.value)"><input value="'+_e(x.name)+'" onchange="_uCCI('+x.id+',\'name\',this.value)"><input type="number" value="'+parseFloat(x.mins_per_unit).toFixed(2)+'" step="0.1" onchange="_uCCI('+x.id+',\'mins_per_unit\',this.value)"><input type="number" value="'+parseFloat(x.ai_rate).toFixed(2)+'" step="0.5" onchange="_uCCI('+x.id+',\'ai_rate\',this.value)"><input type="number" value="'+parseFloat(x.dm_rate).toFixed(2)+'" step="0.5" onchange="_uCCI('+x.id+',\'dm_rate\',this.value)"><input type="number" value="'+parseFloat(x.unit_cost).toFixed(2)+'" step="0.01" onchange="_uCCI('+x.id+',\'unit_cost\',this.value)"><input type="number" value="'+parseFloat(x.min_charge).toFixed(2)+'" step="0.5" onchange="_uCCI('+x.id+',\'min_charge\',this.value)"><button style="background:none;border:none;cursor:pointer;color:#ccc;font-size:15px;padding:0" onclick="_dCCI('+x.id+')">×</button></div>';}).join('')+'</div>';
      }else{
        rowsHTML='<div class="_lc" style="margin-bottom:8px"><div style="display:grid;grid-template-columns:60px 1fr 70px 60px 60px 60px 28px;gap:5px;padding:6px 10px;border-bottom:1px solid #e8e6e2"><span style="font-size:10px;color:#aaa;text-transform:uppercase">Code</span><span style="font-size:10px;color:#aaa;text-transform:uppercase">Name</span><span style="font-size:10px;color:#aaa;text-transform:uppercase">Speed/hr</span><span style="font-size:10px;color:#aaa;text-transform:uppercase">Setup min</span><span style="font-size:10px;color:#aaa;text-transform:uppercase">AI $/hr</span><span style="font-size:10px;color:#aaa;text-transform:uppercase">DM $/hr</span><span></span></div>'+items.map(function(x){return '<div class="_mr" style="grid-template-columns:60px 1fr 70px 60px 60px 60px 28px"><input value="'+_e(x.code||'')+'" onchange="_uCCI('+x.id+',\'code\',this.value)"><input value="'+_e(x.name)+'" onchange="_uCCI('+x.id+',\'name\',this.value)"><input type="number" value="'+parseFloat(x.speed_per_h).toFixed(0)+'" step="1" onchange="_uCCI('+x.id+',\'speed_per_h\',this.value)"><input type="number" value="'+parseFloat(x.setup_min).toFixed(0)+'" step="1" onchange="_uCCI('+x.id+',\'setup_min\',this.value)"><input type="number" value="'+parseFloat(x.ai_rate).toFixed(2)+'" step="0.5" onchange="_uCCI('+x.id+',\'ai_rate\',this.value)"><input type="number" value="'+parseFloat(x.dm_rate).toFixed(2)+'" step="0.5" onchange="_uCCI('+x.id+',\'dm_rate\',this.value)"><button style="background:none;border:none;cursor:pointer;color:#ccc;font-size:15px;padding:0" onclick="_dCCI('+x.id+')">×</button></div>';}).join('')+'</div>';
      }
    }else{
      rowsHTML='<div style="color:#aaa;font-size:13px;padding:10px 0">No items yet.</div>';
    }
    iH=hdr+rowsHTML+'<button onclick="_aCCI('+_cccid+')" style="font-size:13px;color:#378ADD;background:none;border:none;cursor:pointer;padding:6px 0;font-weight:500">+ Add item</button>';
  }
  document.getElementById('_admC').innerHTML='<div class="_sec">'+_bc('Cost Centers')+'<div class="_ml"><div>'+cH+'</div><div>'+iH+'</div></div></div>';
}
...
async function _dCC(id){if(!confirm('Delete cost center and all its items?'))return;await _a('DELETE','/api/cost-centers/'+id);_cccid=null;_rCC();}
async function _aCCI(cid){await _a('POST','/api/cost-centers/items',{cost_center_id:cid,name:'New item'});_rCC();}
